...
function searchByCategory() {
    ...
    searchTimeout = setTimeout(() => {
        ...
        if (!category) {
            document.getElementById('categoryResults').innerHTML = '<h3>Kategória szerinti találatok:</h3><p>Adjon meg egy kategóriát</p>';
            return;
        }

        fetch(`/recipes?category=${encodeURIComponent(category)}`)
                ...
                if (recipes.length === 0) {
                    ...
                    recipes.forEach(recipe => {
                        ...
                        div.innerHTML = `
                            ...
                            ${recipe.category ? `<small>Kategória: ${recipe.category.name}</small>` : ''}
                        `;
                        container.appendChild(div);
                    });
                }
            ...
            .catch(error => {
                ...
                document.getElementById('categoryResults').innerHTML += '<p>Hiba a lekérdezés során.</p>';
            });
